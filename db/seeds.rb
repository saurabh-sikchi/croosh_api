...
known_fors = [
  "Hottest upcoming singer",
  "Instagram sensation",
  "Anchor for Roadies, MTV and many more",
  "Start cricket player of India",
  "Classical singer"
]

Dir.foreach('app/assets/seeds/') do |dirname|
  next if dirname == '.' or dirname == '..'

  profile_video, croosh_vids, profile_pic = nil, [], nil
  Dir.foreach('app/assets/seeds/' + dirname) do |filename|
    next if filename == '.' or filename == '..'

    if filename.end_with?('.mp4')
      if profile_video.nil?
        profile_video = Rails.root.join("app/assets/seeds").join(dirname).join(filename)
      else
        croosh_vids << Rails.root.join("app/assets/seeds").join(dirname).join(filename)
      end
    elsif filename.end_with?('jpg')
      profile_pic = Rails.root.join("app/assets/seeds").join(dirname).join(filename)
    end 
  ...
  celeb.known_for = known_fors[idx]
  celeb.password = 'password123'
  celeb.password_confirmation = 'password123'
  celeb.save!
  ...
  croosh_vids.each do |croosh_vid|
    req_text = req_texts[rand(0...req_texts.size)]
    to_complete_date = Date.today - rand(4..40).days
    croosh = Croosh.create!(
      celeb: celeb,
    ...
    croosh.video.attach(io: File.open(croosh_vid), filename: File.basename(croosh_vid))
  end
